...
import {getAllSellPropertiesOfCity , getAllSellPropertiesOfCityByUsers , getAllSellPropertiesOfCityByAdmin , addNewSavedSearch } from '../../../server_api/Api'
  ...
  // for checkbox
    ...
    useEffect(() => {
        ...
        const getCityData = async () => {
            const {data} = await getAllSellPropertiesOfCity(myCity);
    ...
    const getUsersData = async () => {
        setIsSpinning(true);
        setIsSaved(false)
        const {data} = await getAllSellPropertiesOfCityByUsers(myCity);
            setProperties(data?.AllProperties)
            setIsSpinning(false);
    }
    ...
    const getAdminsData = async () => {
        setIsSpinning(true);
        setIsSaved(false)
        const {data} = await getAllSellPropertiesOfCityByAdmin(myCity);
        setProperties(data?.AllProperties)
        setIsSpinning(false);
    }
    ...
    const onSearch = (value) => {
        ...
        const getData = async () => {
            ...
            setmyCity(newValue)
            const {data} = await getAllSellPropertiesOfCity(newValue);
            setProperties(data?.AllProperties)
            setIsSpinning(false);
        }
        getData();
        ...
        <>
            ...
            <Row style={{marginTop : '20px'}} >
                    <Col xs={24} sm={24} md={{span : 22 , offset : 1}} lg={{span : 12 , offset : 0}} xl={{span: 12, offset : 1}} style={{marginBottom : '15px'}} >
                        <Spin spinning={isSpinning} >
                            <Typography className="propertyLeftSideHead" >All Properties Available For Sell in {city.charAt(0).toUpperCase() + city.slice(1)} are </Typography>
                            {
